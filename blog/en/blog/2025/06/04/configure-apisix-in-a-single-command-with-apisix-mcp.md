...
I have listed some application scenarios for AI large language models. For instance, we can use AI LLMs for interview simulations or language practice. Some companies leverage AI LLMs as intelligent customer service agents. In document writing, students or researchers can use AI LLMs for paper editing, while technical personnel can employ them for organizing technical documentation or formatting. Content creators can generate video scripts using AI. Developers often use AI programming tools like Cursor or Windsurf for code suggestions, code generation, and bug troubleshooting.

Beyond text-based applications, AI LLMs also support multimodal generation, such as creating images or audio/video content. Existing AI LLMs primarily focus on content creation, such as drafting documents and solutions, which we then implement.
...
AI LLMs face two primary limitations: the data silo problem and the lack of agency.

The data silo issue can be likened to "a skilled cook cannot prepare a meal without ingredients." AI LLMs rely on knowledge snapshots from a specific point in the past. For example, in an office setting, if we ask AI to send an email to Manager Zhang, it may not know his identity or email address. In programming, although AI programming tools can assist in developing iOS apps or mini-programs, they may struggle with fitness band applications due to unfamiliarity with the band's system or the frequent updates to its documentation. Additionally, if AI LLMs are only told "high CPU load," they lack the necessary contextual information, such as system process metrics, to effectively assist in resolving the issue.

![Limitations of LLMs](https://static.api7.ai/uploads/2025/06/04/Upw9ofHH_apisix-mcp-practices-1.webp)

The second limitation is the absence of agency—the gap between knowing and doing. AI LLMs are fundamentally content generators. To enable them to perform specific actions, corresponding tools must be provided. For instance, in the first scenario, AI LLMs do not know how to send emails or schedule client meetings. Typically, to execute these tasks, we provide APIs (e.g., Gmail or QQ Mail APIs) and instruct AI to invoke these APIs to send emails to the specified recipients.
...
3. In the content consumption and task confirmation phases, equip AI LLMs with tools. For instance, after sending an email, provide an API to read sent emails, allowing the AI LLM to determine the success of the operation by sending a response.
...
Secondly, OpenAI's Function Calling addresses the issue of AI LLMs invoking tools. With this feature, we can enable AI to call external tools such as APIs or functions, resolving its inability to directly interact with real-world systems. During a conversation with AI, we can specify certain tools. For instance, when sending an email, we can provide an email-sending API along with the recipient's information. AI analyzes the semantics, identifies the need to send an email, invokes the corresponding tool, generates parameters based on the context, and returns the tool's result to the model's context to generate the final response. For example, if the email is sent successfully, it will notify us of success; if it fails, it will inform us of the failure in natural language.
...
Firstly, the technical maturity is insufficient. RAG relies on chunking and vector search, which can disrupt textual context. For example, a Markdown document may originally have a complete introduction and summary, but chunking may only retrieve a portion of it. While knowledge bases seem to provide additional knowledge, their practical utility falls short. Additionally, Function Calling requires predefined input and output structures for APIs, offering low flexibility. In frequently changing business scenarios, such as email sending, it must also integrate with internal system data, resulting in high maintenance costs.

Secondly, the integration cost is high. Whether for RAG or Function Calling, businesses need to modify existing data structures or API architectures. This poses high costs and low returns for small teams with limited technical resources. Furthermore, models are updated and iterated rapidly. While debugging yields satisfactory results, subsequent iterations may enhance model capabilities but degrade previous optimizations, leading to worse query outcomes. Additionally, these models involve cost issues. Function Calling is a closed-source solution, with functionality constrained by a single vendor's ecosystem, making it difficult to expand across models. When enterprises handle sensitive data and prefer not to share it with third-party platforms, they must manage it independently, further increasing integration complexity. Due to these limitations of existing tools, vendors are compelled to explore better solutions.
...
The MCP service ecosystem is already thriving. The following screenshot is from an MCP resource hub, mcp.so. It lists existing MCP services, including file systems, alert systems, automation testing databases, or sending requests, with various brands and manufacturers offering their own MCP services.
...
3. The "Mandarin" of the AI World

- MCP is compatible with multiple models, including Claude, GPT-4, and DeepSeek, without vendor lock-in. Backed by Anthropic, it has industry credibility.
...
The MCP protocol is constantly evolving. After its release by Anthropic, the company continues to advance its development, adding more functionalities to the protocol and ecosystem for businesses, such as publishing identity authentication, cloud connection central registration repositories, and other enterprise-level features. Additionally, Anthropic actively participates in AI conferences and workshops to promote its technology and expand its reach.
...
From the above workflow diagram, when a user makes a request (e.g., to check the weather in Beijing), the system has integrated MCP services to obtain weather information. MCP provides AI with a list of tools, such as `get_weather` or `search_news`, each with corresponding names and descriptions. The AI LLM parses the semantics, matches the most suitable tool (e.g., `get_weather` for a Beijing weather query), and generates parameters based on the predefined input format (e.g., `city: parameter format`).
...
First, click the settings in the top right corner, and on the left sidebar, there is an MCP section that I have pre-configured. If it is empty, click "Add a new global MCP" to navigate to the configuration file. In the "MCP service" field, I added a service named "APISIX-MCP"; you can customize the name.

After configuration, a command needs to be run to start the MCP service. I use the Node.js command-line tool npx for this operation. APISIX's MCP is published to the npm package manager and can be obtained online. You can choose the corresponding tool based on your development language.
...
I set up some scenarios, such as asking APISIX-MCP to "create a route pointing to `https://httpbin.org` with the route ID httpbin, a proxy prefix of `/ip` for requests, and send a request to the gateway to verify whether the configuration is successful." After parsing our semantics, it identifies the need to invoke MCP services to achieve this functionality. Here, it calls a tool with parameters from `create_roots`. We have provided the context, and clicking "run to" confirms the action. In production environments, operational configurations are critical and cannot be changed arbitrarily, hence the need for confirmation. After clicking "run to," we can see the response, understanding the specifics of the API call, including what functions it executes, sends requests to the gateway, and verifies whether the route is successfully created. Clicking "run to" again completes the creation.

![]()

We need not focus on these responses; it will create the route as requested, send a test request to verify, and summarize the actions taken. If configured manually, one might need to set up API keys in the command line or construct complete commands for testing. Should there be an undetected input error, troubleshooting could become challenging.
...
Thirdly, enable the key-auth plugin for the route with ID httpbin. Then, create a consumer named zhihuang with key-auth enabled, generate a high-security random key, and inform me. Send a request to the gateway to verify whether the configuration is successful.
