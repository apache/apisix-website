...
配置完成后，需运行命令以启动 MCP 服务。我通过 Node.js 的命令行工具 npx 来操作，APISIX 的 MCP 已发布到 npm 包管理器，可直接线上获取。大家可根据开发语言选择对应工具。
