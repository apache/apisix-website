...
const NewFeatures: FC = () => {
  ...
  const onClick = useCallback(() => setIdx((i) => (i + 1) % 3), []);

  return (
    <div className={styles.features}>

      <section>
        <h2 className="feat-head-desc"><Translate id="features.component.why.title">Why APISIX Gateway?</Translate></h2>
      ...
      <div className={styles.container} onClick={onClick} role="article">
        {features.map((Feat, i) => <Feat key={`feat${i}`} />)}
      </div>
    </div>
  );
};
