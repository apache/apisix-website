...
|---|---|---|
| Exact response cache | Redis® software stores the body of an HTTP 200 AI response keyed by a normalized effective request. The default TTL is 3,600 seconds and the default maximum response is 1 MiB. | The cache policy supports one Redis® endpoint in 3.18. It does not provide request coalescing, honor upstream `Cache-Control`, or expose a dedicated purge API. A hit reconstructs HTTP 200 and `Content-Type`; other upstream response headers are not stored or replayed. |
