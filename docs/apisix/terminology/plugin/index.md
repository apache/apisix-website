...
> This article introduces the related information of the APISIX Plugin object and how to use it, and introduces how to customize the plugin priority, customize the error response, and dynamically control the execution status of the plugin.

Source: https://apisix.apache.org/docs/apisix/terminology/plugin/
...
A **Plugin** configuration can be bound directly to a [`Route`](/docs/apisix/terminology/route/), [`Service`](/docs/apisix/terminology/service/), [`Consumer`](/docs/apisix/terminology/consumer/) or [`Plugin Config`](/docs/apisix/terminology/plugin-config/). You can refer to [Admin API plugins](/docs/apisix/admin-api/#plugin) for how to use this resource.
...
<div class="code-title">apisix/cli/config.lua</div>
...
1. Plugins in [global rules](/docs/apisix/terminology/global-rule/)
...
However, if the same plugin is configured locally on multiple objects, such as on [Route](/docs/apisix/terminology/route/), [Service](/docs/apisix/terminology/service/), [Consumer](/docs/apisix/terminology/consumer/), [Consumer Group](/docs/apisix/terminology/consumer-group/), or [Plugin Config](/docs/apisix/terminology/plugin-config/), only one copy of configuration is used as each non-global plugin is only executed once. This is because during execution, plugins configured in these objects are merged with respect to a specific order of precedence:
...
APISIX Plugins are hot-loaded. This means that there is no need to restart the service if you add, delete, modify plugins, or even if you update the plugin code. To hot-reload, you can send an HTTP request through the [Admin API](/docs/apisix/admin-api/):
...
For hot-reloading in standalone mode, see the plugin related section in [stand alone mode](/docs/apisix/deployment-modes/#standalone).
