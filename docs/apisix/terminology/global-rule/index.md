...
A global-rule plugin runs before a locally bound plugin in the same phase. However, locally bound `rewrite` handlers run before global-rule `access` handlers. See [Plugins execution order](/docs/apisix/terminology/plugin/#plugins-execution-order) for the complete order.
