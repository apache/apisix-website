...
Source: https://apisix.apache.org/docs/apisix/admin-api/

## Description

The Admin API lets users control their deployed Apache APISIX instance. The [architecture design](/docs/apisix/architecture-design/apisix/) gives an idea about how everything fits together.
...
You can also specify environment variables in yaml keys. This is specifically useful in the `standalone` [mode](/docs/apisix/deployment-modes/#standalone) where you can specify the upstream nodes as follows:

<div class="code-title">./conf/apisix.yaml</div>
...
[Routes](/docs/apisix/terminology/route/) match the client's request based on defined rules, loads and executes the corresponding [plugins](#plugin), and forwards the request to the specified [Upstream](#upstream).
...
| uri              | True, can't be used with `uris`          | Match Rules | Matches the uri. For more advanced matching see [Router](/docs/apisix/terminology/router/).                                                                                                                                                                                                     | "/hello"                                             |
...
| plugins          | False                                    | Plugin      | Plugins that are executed during the request/response cycle. See [Plugin](/docs/apisix/terminology/plugin/) for more.                                                                                                                                                                             |                                                      |
| script           | False                                    | Script      | Used for writing arbitrary Lua code or directly calling existing plugins to be executed. See [Script](/docs/apisix/terminology/script/) for more.                                                                                                                                                 |                                                      |
| upstream         | False                                    | Upstream    | Configuration of the [Upstream](/docs/apisix/terminology/upstream/).                                                                                                                                                                                                                            |                                                      |
| upstream_id      | False                                    | Upstream    | Id of the [Upstream](/docs/apisix/terminology/upstream/) service.                                                                                                                                                                                                                                 |                                                      |
| service_id       | False                                    | Service     | Configuration of the bound [Service](/docs/apisix/terminology/service/).                                                                                                                                                                                                                          |                                                      |
| plugin_config_id | False, can't be used with `script`       | Plugin      | [Plugin config](/docs/apisix/terminology/plugin-config/) bound to the Route.                                                                                                                                                                                                                      |                                                      |
...
| service_name                | True, can't be used with `nodes`                                 | String                        | Service name used for [service discovery](/docs/apisix/discovery/).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `a-bootiful-client`                                                                                                                        |
| discovery_type              | True, if `service_name` is used                                  | String                        | The type of service [discovery](/docs/apisix/discovery/).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `eureka`                                                                                                                                   |
...
| checks                      | False                                                            | Health Checker                | Configures the parameters for the [health check](/docs/apisix/tutorials/health-check/).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |                                                                                                                                            |
...
The features described below requires APISIX to be run on [APISIX-Runtime](/docs/apisix/FAQ/#how-do-i-build-the-apisix-runtime-environment):
...
| Parameter    | Required | Type                     | Description                                                                                                    | Example                                          |
| ------------ | -------- | ------------------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| cert         | True     | Certificate              | HTTPS certificate. This field supports saving the value in Secret Manager using the [APISIX Secret](/docs/apisix/terminology/secret/) resource.                                                                                             |                                                  |
| key          | True     | Private key              | HTTPS private key. This field supports saving the value in Secret Manager using the [APISIX Secret](/docs/apisix/terminology/secret/) resource.                                                                                             |                                                  |
| certs        | False    | An array of certificates | Used for configuring multiple certificates for the same domain excluding the one provided in the `cert` field. This field supports saving the value in Secret Manager using the [APISIX Secret](/docs/apisix/terminology/secret/) resource.  |                                                  |
| keys         | False    | An array of private keys | Private keys to pair with the `certs`. This field supports saving the value in Secret Manager using the [APISIX Secret](/docs/apisix/terminology/secret/) resource.                                                                   |                                                  |
...
| ssl_protocols | False    | An array of ssl protocols               | It is used to control the SSL/TLS protocol version used between servers and clients. See [SSL Protocol](/docs/apisix/ssl-protocol/) for more examples.                  |                `["TLSv1.1", "TLSv1.2", "TLSv1.3"]`                                  |
...
See [Certificate](/docs/apisix/certificate/) for more examples.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------ | ---------- |
| plugins     | True     | Plugins that are executed during the request/response cycle. See [Plugin](/docs/apisix/terminology/plugin/) for more. |            |
...
Route used in the [Stream Proxy](/docs/apisix/stream-proxy/).
...
| upstream    | False    | Upstream | Configuration of the [Upstream](/docs/apisix/terminology/upstream/). |                               |
| upstream_id | False    | Upstream | Id of the [Upstream](/docs/apisix/terminology/upstream/) service.      |                               |
| service_id  | False    | String   | Id of the [Service](/docs/apisix/terminology/service/) service.        |                               |
...
To learn more about filtering in stream proxies, check [this](/docs/apisix/stream-proxy/#more-route-match-options) document.
...
See [grpc-transcode plugin](/docs/apisix/plugins/grpc-transcode/#enabling-the-plugin) doc for more examples.
...
| Parameter | Required | Type      | Description                          | Example                       |
|-----------|----------|-----------|--------------------------------------| ----------------------------- |
| content   | True     | String    | Content of `.proto` or `.pb` files   | See [here](/docs/apisix/plugins/grpc-transcode/#enabling-the-plugin)         |
