...
Source: https://apisix.apache.org/docs/apisix/FAQ/
...
- Fully dynamic.
- Supports [hot loading of Plugins](/docs/apisix/terminology/plugin/#hot-reload).
...
Specific results of the performance benchmarks can be found [here](/docs/apisix/benchmark/).
...
You can write your own Plugins by referring to [How to write your own Plugins](/docs/apisix/plugin-develop/).
...
1. Using the `vars` field in a [Route](/docs/apisix/terminology/route/):
...
2. Using the [traffic-split](/docs/apisix/plugins/traffic-split/) Plugin.
...
1. Setting `http_to_https` to `true` in the [redirect](/docs/apisix/plugins/redirect/) Plugin:
...
You can learn more about hot reloading of Plugins [here](/docs/apisix/terminology/plugin/#hot-reload).
...
This is caused by the missing OpenResty openssl development kit. To install it, refer [installing dependencies](/docs/apisix/install-dependencies/).
...
To strip a prefix from a path in your route, like to take `/foo/get` and strip it to `/get`, you can use the [proxy-rewrite](/docs/apisix/plugins/proxy-rewrite/) Plugin:
