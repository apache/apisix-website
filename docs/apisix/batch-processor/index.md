...
Source: https://apisix.apache.org/docs/apisix/batch-processor/
...
Every logger built on the batch processor therefore accepts a `max_pending_entries` limit through its [plugin metadata](/docs/apisix/terminology/plugin-metadata/), which defaults to `8192`. While the backlog exceeds the limit, new entries are discarded and a summary is written to the error log at most once per second:
