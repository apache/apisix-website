...
Source: https://apisix.apache.org/docs/apisix/plugins/ai-rate-limiting/

## Description

The `ai-rate-limiting` Plugin enforces token-based rate limiting for requests sent to LLM services. It helps manage API usage by controlling the number of tokens consumed within a specified time frame, ensuring fair resource allocation and preventing excessive load on the service. It is often used with [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) or [`ai-proxy-multi`](/docs/apisix/plugins/ai-proxy-multi/) Plugin.
