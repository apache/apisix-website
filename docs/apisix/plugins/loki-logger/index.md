...
Source: https://apisix.apache.org/docs/apisix/plugins/loki-logger/
...
When enabled, the Plugin will serialize the request context information to [JSON objects](https://grafana.com/docs/loki/latest/api/#push-log-entries-to-loki) and add them to the queue, before they are pushed to Loki. See [batch processor](/docs/apisix/batch-processor/) for more details.
...
| name | string | False    | loki-logger | | Unique identifier of the Plugin for the batch processor. If you use [Prometheus](/docs/apisix/plugins/prometheus/) to monitor APISIX metrics, the name is exported in `apisix_batch_process_entries`. |
...
The following example demonstrates how you can customize log format using [Plugin Metadata](/docs/apisix/terminology/plugin-metadata/).
