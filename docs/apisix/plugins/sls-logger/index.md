# Alibaba Cloud SLS Logger (sls-logger)

> This document contains information about the Apache APISIX sls-logger Plugin.

Source: https://apisix.apache.org/docs/apisix/plugins/sls-logger/
...
It might take some time to receive the log data. It will be automatically sent after the timer function in the [batch processor](/docs/apisix/batch-processor/) expires.
...
| log_format       | False    | Log format declared as key-value pairs in JSON. Values support strings and nested objects (up to five levels deep; deeper fields are truncated). Within strings, [APISIX](/docs/apisix/apisix-variable/) or [NGINX](http://nginx.org/en/docs/varindex.html) variables can be referenced by prefixing with `$`. |
...
| ssl_verify        | False    | When set to `true`, verifies the TLS certificate of the log server. Defaults to `true`.                                                                                                                                                          |
...
NOTE: `encrypt_fields = {"access_key_secret"}` is also defined in the schema, which means that the field will be stored encrypted in etcd. See [encrypted storage fields](/docs/apisix/plugin-develop/#encrypted-storage-fields).
...
![sls logger view](https://raw.githubusercontent.com/apache/apisix/master/docs/assets/images/plugin/sls-logger-1.png "sls logger view")
