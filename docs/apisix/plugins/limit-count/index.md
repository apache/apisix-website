# Rate Limiting by Count (limit-count)
...
Source: https://apisix.apache.org/docs/apisix/plugins/limit-count/
...
NOTE: `encrypt_fields = {"redis_password", "sentinel_password"}` is also defined in the schema, which means that the fields will be stored encrypted in etcd. See [encrypted storage fields](/docs/apisix/plugin-develop/#encrypted-storage-fields).
...
<div class="code-title">limit-count-ic.yaml</div>
...
The [`proxy-rewrite`](/docs/apisix/plugins/proxy-rewrite/) plugin is used to rewrite the URI to `/get` so that requests are forwarded to the correct endpoint.
...
The following example demonstrates how you can configure different rate limiting policies for regular and anonymous Consumers, where the anonymous Consumer does not need to authenticate and has less quota. While this example uses [`key-auth`](/docs/apisix/plugins/key-auth/) for authentication, the anonymous Consumer can also be configured with [`basic-auth`](/docs/apisix/plugins/basic-auth/), [`jwt-auth`](/docs/apisix/plugins/jwt-auth/), and [`hmac-auth`](/docs/apisix/plugins/hmac-auth/).
...
<div class="code-title">gatewayproxy.yaml</div>
