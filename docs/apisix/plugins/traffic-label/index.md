...
> The traffic-label Plugin sets request headers based on configurable matching rules with weighted distribution, enabling traffic tagging and canary deployments.

Source: https://apisix.apache.org/docs/apisix/plugins/traffic-label/

## Description

The `traffic-label` Plugin sets request headers based on configurable matching rules. Similar to the [workflow](/docs/apisix/plugins/workflow/) Plugin, it evaluates rules in order and executes an action on the first match. The key difference is that `traffic-label` supports **weighted distribution** within each rule's action list, enabling proportional traffic labeling for canary deployments and A/B testing.
