...
Source: https://apisix.apache.org/docs/apisix/plugins/file-logger/
...
- `file-logger` plugin can count request and response data for individual routes locally, which is useful for [debugging](/docs/apisix/debug-mode/).
- `file-logger` plugin can get [APISIX variables](/docs/apisix/apisix-variable/) and [NGINX variables](http://nginx.org/en/docs/varindex.html), while `access.log` can only use NGINX variables.
