...
> The mqtt-proxy Plugin supports proxying and load balancing MQTT requests to MQTT servers, supporting MQTT versions 3.1.x and 5.0.

Source: https://apisix.apache.org/docs/apisix/plugins/mqtt-proxy/
...
<div class="code-title">mqtt-proxy-ic.yaml</div>
...
Stream proxies use TCP connections and can accept TLS. Follow the guide about [how to accept TLS over TCP connections](/docs/apisix/stream-proxy/#accept-tls-over-tcp-connection) to open a stream proxy with enabled TLS.
...
Configure `ssl` providing the CA certificate and the server certificate, together with a list of SNIs. Steps to protect `stream_routes` with `ssl` are equivalent to the ones to [protect Routes](/docs/apisix/mtls/#protect-route).
