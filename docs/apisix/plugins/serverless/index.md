...
> The serverless plugins, `serverless-pre-function` and `serverless-post-function`, allow you to dynamically run Lua functions at specified phases in APISIX.

Source: https://apisix.apache.org/docs/apisix/plugins/serverless/

## Description

The serverless functions consist of two plugins, `serverless-pre-function` and `serverless-post-function`. These plugins enable the execution of user-defined logic at the beginning and end of the [execution phases](/docs/apisix/terminology/plugin/#plugins-execution-lifecycle) the functions hook to.
...
The example below demonstrates how you can configure the serverless plugins to execute custom logics to log information to error logs before and after the `rewrite` [phase](/docs/apisix/terminology/plugin/#plugins-execution-lifecycle).
...
<div class="code-title">serverless-functions-ic.yaml</div>
...
Create a [Service](/docs/apisix/terminology/service/) with a serverless function to register a custom variable `a6_route_labels`, enable a logging plugin to later log the custom variable, and configure an upstream:
...
Next, update the log format for all `syslog` instances with the new variable by configuring the [Plugin metadata](/docs/apisix/terminology/plugin/#plugin-metadata):
...
<div class="code-title">serverless-remove-body-ic.yaml</div>
...
For simpler response modifications, such as modifying HTTP status codes, request headers, or the entire response body, please use the [`response-rewrite`](/docs/apisix/plugins/response-rewrite/) plugin.
