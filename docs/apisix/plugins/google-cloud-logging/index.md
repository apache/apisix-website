...
Source: https://apisix.apache.org/docs/apisix/plugins/google-cloud-logging/
...
| name                    | string        | False    | google-cloud-logging                                                                                                                                                                                 | | Unique identifier of the Plugin for the batch processor. If you use [Prometheus](/docs/apisix/plugins/prometheus/) to monitor APISIX metrics, the name is exported in `apisix_batch_process_entries`.                                                                                                                 |
...
NOTE: `encrypt_fields = {"auth_config.private_key"}` is also defined in the schema, which means that the field will be stored encrypted in etcd. See [encrypted storage fields](/docs/apisix/plugin-develop/#encrypted-storage-fields).
...
| Name               | Type    | Required | Default | Valid values | Description                                                                                                                                                                                                                                                                     |
|--------------------|---------|----------|---------|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| log_format         | object  | False    |         |              | Custom log format using key-value pairs in JSON format. Values can reference [APISIX variables](/docs/apisix/apisix-variable/) or [NGINX variables](https://nginx.org/en/docs/http/ngx_http_core_module.html) by prefixing with `$`. This configuration is global and applies to all Routes and Services that use the `google-cloud-logging` Plugin. |
| log_format_extra         | object  | False    |         |              | Extra log fields **added on top of** the default log entry, keeping every default field instead of replacing them (unlike `log_format`). Same value syntax as `log_format`. Ignored when `log_format` is set. |
