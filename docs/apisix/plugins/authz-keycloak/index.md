...
Source: https://apisix.apache.org/docs/apisix/plugins/authz-keycloak/
...
| client_secret                                | string        | False    |                                               |                                                                        | The client secret, if required. You can use APISIX Secret to store and reference this value. APISIX currently supports storing secrets in two ways: [Environment Variables and HashiCorp Vault](/docs/apisix/terminology/secret/).                            |
...
NOTE: `encrypt_fields = {"client_secret"}` is also defined in the schema, which means that the field will be stored encrypted in etcd. See [encrypted storage fields](/docs/apisix/plugin-develop/#encrypted-storage-fields).
...
<div class="code-title">keycloak.yaml</div>
...
<div class="code-title">authz-keycloak-ic.yaml</div>
