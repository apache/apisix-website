...
> The ai-cache Plugin caches LLM responses in Redis and replays them for later requests that resolve to the same prompt, cutting upstream token cost and latency.

Source: https://apisix.apache.org/docs/apisix/plugins/ai-cache/
...
The `ai-cache` Plugin must be used with the [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) or [`ai-proxy-multi`](/docs/apisix/plugins/ai-proxy-multi/) Plugin.
...
Create a Route to the LLM chat completion endpoint with the [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) and `ai-cache` Plugins.
...
<div class="code-title">ai-cache-ic.yaml</div>
