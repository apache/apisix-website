...
> The ai-lakera-guard Plugin integrates Apache APISIX with the Lakera Guard API (v2) to scan LLM requests for prompt injection, jailbreak, PII, content-policy violations, and malicious links, then blocks or alerts on Lakera's verdict.

Source: https://apisix.apache.org/docs/apisix/plugins/ai-lakera-guard/
...
The `ai-lakera-guard` Plugin should be used with either the [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) or [`ai-proxy-multi`](/docs/apisix/plugins/ai-proxy-multi/) Plugin for proxying LLM requests. It relies on the context that `ai-proxy` populates to extract chat content in a protocol-aware way.
...
|------|------|----------|---------|--------------|-------------|
| api_key | string | True | | | Lakera Guard API key, sent as `Authorization: Bearer`. The value is encrypted with AES before being stored in etcd, and supports [secret references](/docs/apisix/terminology/secret/) (`$secret://`) and environment variables (`$env://`). |
...
Create a Route to the LLM chat completion endpoint using the [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) Plugin and configure the `ai-lakera-guard` Plugin:
...
Create a Route with the `ai-lakera-guard` and [`ai-proxy`](/docs/apisix/plugins/ai-proxy/) Plugins configured as such:
...
<div class="code-title">ai-lakera-guard-ic.yaml</div>
