...
> The acl Plugin implements label-based access control for API routes, allowing or denying requests based on consumer labels or external user attributes.

Source: https://apisix.apache.org/docs/apisix/plugins/acl/

## Description

The `acl` Plugin provides label-based access control for API routes. It checks consumer labels (from APISIX [Consumers](/docs/apisix/terminology/consumer/)) or external user attributes (from authentication plugins that set `ctx.external_user`) against configured allow or deny lists.
...
The example below demonstrates how to use the `acl` Plugin with [`key-auth`](/docs/apisix/plugins/key-auth/) to allow only consumers that have a specific label value.
