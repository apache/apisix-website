...
async function replaceMDElements(project, path, branch = 'master', locale = 'en') {
  ...
  // the current-version Astro pages generated by next/scripts/sync-content.mjs.
  const chineseCanonicalOptions = {
    files: allMDFilePaths,
    from: /(<link\s+rel=["']canonical["']\s+href=["'])https:\/\/docs\.api7\.ai\//g,
      ...
      task: async () => {
        ...
        await Promise.allSettled([
          copyDocs(enSrcDocs, enTargetDocs)
            .then(() => replaceMDElements(projectName, [enTargetDocs], branchName, 'en'))
          ...
          copyDocs(zhSrcDocs, zhTargetDocs)
            .then(() => replaceMDElements(projectName, [zhTargetDocs], branchName, 'zh')),
        ]).catch(() => {
          /* ignore */
        });
      },
