...
Source: https://apisix.apache.org/zh/docs/apisix/batch-processor/
...
因此所有基于批处理器的日志插件都通过[插件元数据](/zh/docs/apisix/terminology/plugin-metadata/)提供 `max_pending_entries` 上限，默认值为 `8192`。积压超过该上限期间新条目会被丢弃，并且每秒最多向错误日志输出一条汇总信息：
