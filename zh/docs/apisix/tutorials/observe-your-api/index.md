...
> 本文介绍了 API 网关 Apache APISIX 可观察性插件并了解如何设置这些插件。

Source: https://apisix.apache.org/zh/docs/apisix/tutorials/observe-your-api/
...
在进行该教程之前，请确保你已经[公开服务](/zh/docs/apisix/tutorials/expose-api/)。
...
- [http-logger](/zh/docs/apisix/plugins/http-logger/)
- [skywalking-logger](/zh/docs/apisix/plugins/skywalking-logger/)
- [tcp-logger](/zh/docs/apisix/plugins/tcp-logger/)
- [kafka-logger](/zh/docs/apisix/plugins/kafka-logger/)
- [rocketmq-logger](/zh/docs/apisix/plugins/rocketmq-logger/)
- [udp-logger](/zh/docs/apisix/plugins/udp-logger/)
- [clickhouse-logger](/zh/docs/apisix/plugins/clickhouse-logger/)
- [error-logger](/zh/docs/apisix/plugins/error-log-logger/)
- [google-cloud-logging](/zh/docs/apisix/plugins/google-cloud-logging/)

你可以在 APISIX [插件中心](/zh/docs/apisix/plugins/http-logger/) 查看 APISIX 支持的所有日志插件。接下来我们将使用 `http-logger` 插件为你演示如何将 APISIX 的日志数据发送到 HTTP/HTTPS 服务器中。
...
**指标**是在⼀段时间内测量的数值。与⽇志不同，指标在默认情况下是结构化的，这使得查询和优化存储变得更加容易。而 APISIX 也提供了 [Prometheus](/zh/docs/apisix/plugins/prometheus/) 的插件来获取你的 API 指标，并在 Prometheus 中暴露它们。通过使用 APISIX 提供的 Grafana 仪表板元数据，并从 Prometheus 中获取指标，更加方便地监控你的 API。
...
- [Node status 插件](/zh/docs/apisix/plugins/node-status/)(https://apisix.apache.org/docs/apisix/plugins/node-status/)
- [Datadog 插件](/zh/docs/apisix/plugins/datadog/)

## 链路追踪

**链路追踪**就是将一次请求还原成调用链路，并将该请求的调用情况使用拓扑的方式展现，比如展示各个微服务节点上的耗时，请求具体经过了哪些服务器以及每个服务节点的请求状态等内容。

[Zipkin](https://zipkin.io/) 一个开源的分布式追踪系统。APISIX 的[zipkin 插件](/zh/docs/apisix/plugins/zipkin/) 支持根据 [Zipkin API 规范](https://zipkin.io/pages/instrumenting.html) 收集链路信息并报告给 Zipkin Collector。
...
- [Skywalking 插件](/zh/docs/apisix/plugins/skywalking/)

- [OpenTelemetry 插件](/zh/docs/apisix/plugins/opentelemetry/)
