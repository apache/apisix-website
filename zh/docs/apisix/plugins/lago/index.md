...
> The lago plugin reports usage to a Lago instance, which allows users to integrate Lago with APISIX for API monetization.

Source: https://apisix.apache.org/zh/docs/apisix/plugins/lago/
...
While APISIX performs retries based on the [batch processor](/docs/apisix/batch-processor/) configuration, you don't need to worry about duplicate events being reported to Lago.
...
Technically, the logic is executed in the NGINX log phase and [batch processor](/docs/apisix/batch-processor/) timer, so this does not affect the request itself.
