...
> 本文将介绍 prometheus 插件，以及将 APISIX 与 Prometheus 集成以进行指标收集和持续监控。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/prometheus/
...
你可以通过插件的[元数据（Plugin Metadata）](/zh/docs/apisix/terminology/plugin-metadata/)进行配置。元数据通过 Admin API 动态设置，无需重启即可在运行时生效。
...
[`ai-cache`](/zh/docs/apisix/plugins/ai-cache/) 插件导出以下四个指标：
...
接下来，使用 [`public-api`](/zh/docs/apisix/plugins/public-api/) 插件创建一个路由，并为 APISIX 指标公开一个公共 API 端点：
...
以下示例演示如何通过[插件元数据（Plugin Metadata）](/zh/docs/apisix/terminology/plugin-metadata/)将选定内置标签的值折叠为空字符串 `""`，从而降低指标基数。这在 Kubernetes 弹性伸缩等动态环境中尤其有用：此时上游节点 IP（`node` 标签）频繁变化，否则会很快撑爆 `prometheus-metrics` 共享字典。
