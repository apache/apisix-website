...
> basic-auth 插件为消费者添加了基本访问身份验证，以便消费者在访问上游资源之前进行身份验证。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/basic-auth/

## 描述

`basic-auth` 插件为[消费者](/zh/docs/apisix/terminology/consumer/)添加了[基本访问身份验证](https://en.wikipedia.org/wiki/Basic_access_authentication)，以便消费者在访问上游资源之前进行身份验证。
