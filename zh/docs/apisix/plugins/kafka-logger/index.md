...
> kafka-logger 插件将请求和响应日志作为 JSON 对象批量推送到 Apache Kafka 集群，并支持自定义日志格式以便更好地管理数据。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/kafka-logger/

## 描述

`kafka-logger` 插件将请求和响应日志作为 JSON 对象批量推送到 Apache Kafka 集群，并支持自定义日志格式。

接收日志数据可能需要一些时间。数据将在 [批处理器](/zh/docs/apisix/batch-processor/) 中的计时器函数到期后自动发送。
...
| log_format                       | object  | 否       |                 |                                                   | 以 JSON 键值对声明的日志格式。值支持字符串和嵌套对象（最多五层，超出部分将被截断）。字符串中可通过在前面加上 `$` 来引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 或 [NGINX 内置变量](http://nginx.org/en/docs/varindex.html)。                                                   |
...
| api_version                      | integer | 否       | 1               | [0, 1, 2]                                         | 对应 [lua-resty-kafka](https://github.com/doujiang24/lua-resty-kafka) 中的 `api_version` 参数，用于指定 Kafka Produce API 的版本。设置为 `2` 时 broker 才会存储消息的时间戳，否则时间戳为 `-1`（部分消费端会显示为 `1970-01-01`）。需要 Kafka 0.10 及以上版本。               |

该插件支持使用批处理器来聚合并批量处理条目（日志/数据），避免频繁提交数据。默认情况下，批处理器每 `5` 秒或队列中的数据达到 `1000` 条时提交数据。如需了解批处理器相关参数设置，请参考[批处理器](/zh/docs/apisix/batch-processor/#配置)配置部分。
...
以下示例演示了如何使用[插件元数据](/zh/docs/apisix/terminology/plugin-metadata/)和[内置变量](/zh/docs/apisix/apisix-variable/)自定义日志格式，以记录请求和响应中的特定头字段。
