...
> 本篇文档介绍了 Apache APISIX feishu-auth 插件的相关信息。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/feishu-auth/
...
注意：schema 中定义了 `encrypt_fields = {"app_secret", "secret", "secret_fallbacks"}`，这意味着这些字段将会被加密存储在 etcd 中。具体参考[加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。
