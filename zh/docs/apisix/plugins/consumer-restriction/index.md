...
Source: https://apisix.apache.org/zh/docs/apisix/plugins/consumer-restriction/

## 描述

`consumer-restriction` 插件基于消费者名称、路由 ID、服务 ID 或消费者组 ID 实现访问控制。

该插件需要与认证插件配合使用，例如 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 和 [`jwt-auth`](/zh/docs/apisix/plugins/jwt-auth/)，这意味着在使用场景中至少需要创建一个[消费者](/zh/docs/apisix/terminology/consumer/)。详情请参见下方示例。
...
示例中使用 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 作为认证方式，你可以根据需要灵活调整为其他认证插件。
...
以下示例演示如何在路由上使用 `consumer-restriction` 插件，按消费者名称限制消费者访问，消费者通过 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 进行认证。
...
以下示例演示如何在路由上使用 `consumer-restriction` 插件，按消费者名称和 HTTP 方法限制消费者访问，消费者通过 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 进行认证。
...
以下示例演示如何使用 `consumer-restriction` 插件按服务 ID 限制消费者访问，消费者通过 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 进行认证。
