...
> traffic-label 插件根据可配置的匹配规则和权重分发设置请求头，实现流量染色与灰度发布。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/traffic-label/

## 描述

`traffic-label` 插件根据可配置的匹配规则为请求设置头部信息。与 [workflow](/zh/docs/apisix/plugins/workflow/) 插件类似，该插件按顺序对规则进行求值，并对第一个匹配的规则执行动作。两者的关键区别在于：`traffic-label` 支持在每条规则的动作列表中设置**权重分发**，从而实现按比例的流量染色，适用于灰度发布和 A/B 测试场景。
