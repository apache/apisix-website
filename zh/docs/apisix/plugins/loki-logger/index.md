...
> loki-logger 插件通过 Loki HTTP API /loki/api/v1/push 将请求和响应日志批量推送到 Grafana Loki。该插件还支持自定义日志格式。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/loki-logger/
...
启用后，插件会将请求上下文信息序列化为 [JSON object](https://grafana.com/docs/loki/latest/api/#push-log-entries-to-loki) 并将其添加到队列中，然后再将其推送到 Loki。有关更多详细信息，请参阅批处理处理器 [batch processor](/zh/docs/apisix/batch-processor/)。
...
| name | string | 否 | loki-logger | | 批处理器插件的唯一标识符。如果使用 [Prometheus](/zh/docs/apisix/plugins/prometheus/) 监控 APISIX 指标，则名称会导出到 `apisix_batch_process_entries`。 |
...
该插件支持使用批处理器对条目（日志/数据）进行批量聚合和处理，避免了频繁提交数据的需求。批处理器每隔 `5` 秒或当队列中的数据达到 `1000` 时提交数据。有关更多信息或设置自定义配置，请参阅 [批处理器](/zh/docs/apisix/batch-processor/#configuration)。

## Plugin Metadata

你还可以使用 [Plugin Metadata](/zh/docs/apisix/terminology/plugin-metadata/) 全局配置日志格式，该 Plugin Metadata 配置所有 `loki-logger` 插件实例的日志格式。如果在单个插件实例上配置的日志格式与在 Plugin Metadata 上配置的日志格式不同，则在单个插件实例上配置的日志格式优先。

| 名称 | 类型 | 必选项 | 默认值 | 有效值 | 描述 |
|------|------|--------|--------|--------|------|
| log_format | object | 否 |  |  | 日志格式以 JSON 的键值对声明。值支持字符串和嵌套对象（最多五层，超出部分将被截断）。字符串中可通过在前面加上 `$` 来引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 和 [NGINX 变量](http://nginx.org/en/docs/varindex.html)。 |
...
以下示例演示了如何使用 [Plugin Metadata](/zh/docs/apisix/terminology/plugin-metadata/) 自定义日志格式。
