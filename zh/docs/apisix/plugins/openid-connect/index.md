...
Source: https://apisix.apache.org/zh/docs/apisix/plugins/openid-connect/
...
注意：schema 中还定义了 `encrypt_fields = {"client_secret", "client_rsa_private_key", "dpop.private_key"}`，这意味着这些字段将在 etcd 中加密存储。详见[加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。

此外，你可以使用环境变量或 APISIX Secret 来存储和引用插件属性。APISIX 目前支持两种存储密钥的方式——[环境变量和 HashiCorp Vault](/zh/docs/apisix/terminology/secret/)。
...
详见[实现授权码授权](/zh/docs/apisix/tutorials/keycloak-oidc/#实现-authorization-code-grant)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用授权码流程的完整示例。
...
详见[实现授权码授权](/zh/docs/apisix/tutorials/keycloak-oidc/#实现-authorization-code-grant)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用带 PKCE 的授权码流程的示例。
...
详见[实现客户端凭证授权](/zh/docs/apisix/tutorials/keycloak-oidc/#实现-client-credential-grant)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用客户端凭证流程的示例。
...
详见[实现客户端凭证授权](/zh/docs/apisix/tutorials/keycloak-oidc/#实现-client-credential-grant)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用带令牌内省的客户端凭证流程的示例。
...
详见[实现密码授权](/zh/docs/apisix/tutorials/keycloak-oidc/#实施-password-grant)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用密码流程的示例。
...
详见[刷新令牌](/zh/docs/apisix/tutorials/keycloak-oidc/#refresh-token)，获取使用 `openid-connect` 插件与 Keycloak 集成并使用带令牌刷新的密码流程的示例。
...
如果你以[独立模式](/zh/docs/apisix/deployment-modes/#standalone-mode)部署 APISIX，请确保配置了 `session.secret`。
