...
> 本篇文档介绍了 Apache APISIX ldap-auth 插件的相关信息。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/ldap-auth/
...
| 名称    | 类型   | 必选项 | 描述                                                                      |
| ------- | ------ | -------- | -------------------------------------------------------------------------------- |
| user_dn | string | 是     | LDAP 客户端的 dn，例如：`cn=user01,ou=users,dc=example,dc=org`。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。 |
