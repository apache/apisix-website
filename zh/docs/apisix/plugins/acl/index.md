...
> acl 插件基于标签实现访问控制，通过检查消费者标签或外部用户属性来允许或拒绝请求。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/acl/

## 描述

`acl` 插件为 API 路由提供基于标签的访问控制。它检查 APISIX [消费者](/zh/docs/apisix/terminology/consumer/)的标签，或来自外部认证插件（设置了 `ctx.external_user`）的用户属性，并与配置的允许列表或拒绝列表进行比对。
...
以下示例演示如何将 `acl` 插件与 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 结合使用，仅允许具有特定标签值的消费者访问。
