...
> workflow 插件支持根据给定的一组规则有条件地执行对客户端流量的用户定义操作。这提供了一种实现复杂流量管理的细粒度方法。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/workflow/
...
| rules.actions | array[array] | 是 | | | 条件匹配成功后要执行的操作的数组。目前数组只支持一个操作，必须是 `return`、`limit-count` 或 `limit-conn`。当操作配置为 `return` 时，可以配置条件匹配成功时返回给客户端的 HTTP 状态码。当操作配置为 `limit-count` 时，可以配置 [`limit-count`](/zh/docs/apisix/plugins/limit-count/) 插件除 `group` 之外的所有选项。当操作配置为 `limit-conn` 时，可以配置 [`limit-conn`](/zh/docs/apisix/plugins/limit-conn/) 插件的所有选项。 |
...
虽然此示例将使用 [`key-auth`](/zh/docs/apisix/plugins/key-auth/)，但您可以轻松地将其替换为其他身份验证插件。
