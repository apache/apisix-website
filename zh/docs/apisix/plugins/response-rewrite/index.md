...
> response-rewrite 插件提供了重写 APISIX 及其上游服务返回给客户端的响应的选项。使用该插件，你可以修改 HTTP 状态代码、请求标头、响应正文等。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/response-rewrite/
...
如果你仅需要重定向功能，建议使用 [redirect](/zh/docs/apisix/plugins/redirect/) 插件。
...
以下示例通过使用 `key-auth` 插件配置插件，演示了 `response-rewrite` 插件与[执行阶段](/zh/docs/apisix/terminology/plugin/#plugins-execution-lifecycle)之间的联系，并查看在未经身份验证的请求的情况下，响应仍如何重写为 `200 OK`。
