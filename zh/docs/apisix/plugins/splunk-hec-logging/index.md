...
> splunk-hec-logging 插件将请求和响应上下文信息序列化为 Splunk Event Data 格式并批量推送到 Splunk HTTP Event Collector（HEC），支持自定义日志格式以增强数据管理。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/splunk-hec-logging/
...
| 名称               | 类型    | 必选项 | 默认值 | 有效值 | 描述                                                                                                                                                                                                 |
|--------------------|---------|--------|--------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| log_format         | object  | False  |        |        | 以 JSON 键值对形式声明的自定义日志格式。值可通过 `$` 前缀引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 或 [NGINX 变量](https://nginx.org/en/docs/http/ngx_http_core_module.html)。该配置全局生效，对所有绑定 `splunk-hec-logging` 的路由和服务生效。 |
