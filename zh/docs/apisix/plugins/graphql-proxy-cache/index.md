...
> graphql-proxy-cache 插件缓存 GraphQL 查询的响应，支持磁盘和内存两种缓存策略，对包含 mutation 操作的请求自动绕过缓存。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/graphql-proxy-cache/
...
本插件复用 [`proxy-cache`](/zh/docs/apisix/plugins/proxy-cache/) 插件的缓存基础设施。启用本插件前，需要先在 `config.yaml` 中配置缓存区域。
...
首先使用 [`public-api`](/zh/docs/apisix/plugins/public-api/) 插件创建一个路由来暴露清除接口：
