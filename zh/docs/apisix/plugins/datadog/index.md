...
> datadog 插件与 Datadog 集成，将指标批量发送到 DogStatsD，助力 API 监控与性能追踪。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/datadog/
...
该插件支持通过 UDP 将指标以批量方式推送到外部 Datadog 代理。数据可能需要一段时间才能被接收，会在[批处理器](/zh/docs/apisix/batch-processor/)的定时器到期后自动发送。
...
本插件支持使用批处理器来聚合并批量处理条目（日志/数据），避免频繁提交数据。默认情况下，批处理器每 `5` 秒或队列中数据达到 `1000` 条时提交一次。详情及自定义配置请参考[批处理器](/zh/docs/apisix/batch-processor/#配置)。
