...
Source: https://apisix.apache.org/zh/docs/apisix/plugins/limit-count/
...
注意：schema 中还定义了 `encrypt_fields = {"redis_password", "sentinel_password"}`，这意味着这些字段将会被加密存储在 etcd 中。具体参考[加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。
...
[`proxy-rewrite`](/zh/docs/apisix/plugins/proxy-rewrite/) 插件用于将 URI 重写为 `/get`，以便将请求转发到正确的端点。
...
以下示例演示了如何为常规和匿名消费者配置不同的速率限制策略，其中匿名消费者不需要进行身份验证并且配额较少。虽然此示例使用 [`key-auth`](/zh/docs/apisix/plugins/key-auth/) 进行身份验证，但匿名消费者也可以使用 [`basic-auth`](/zh/docs/apisix/plugins/basic-auth/)、[`jwt-auth`](/zh/docs/apisix/plugins/jwt-auth/) 和 [`hmac-auth`](/zh/docs/apisix/plugins/hmac-auth/) 进行配置。
