...
> ai-lakera-guard 插件将 Apache APISIX 与 Lakera Guard API（v2）集成，用于扫描 LLM 请求中的提示词注入、越狱、PII、内容策略违规以及恶意链接，并根据 Lakera 的判定结果拦截或告警。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/ai-lakera-guard/
...
`ai-lakera-guard` 插件应与 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 或 [`ai-proxy-multi`](/zh/docs/apisix/plugins/ai-proxy-multi/) 插件配合使用以代理 LLM 请求。它依赖 `ai-proxy` 填充的上下文，以协议感知的方式提取对话内容。
...
|------|------|--------|--------|--------|------|
| api_key | string | 是 | | | Lakera Guard API 密钥，以 `Authorization: Bearer` 形式发送。该值在存储到 etcd 之前会使用 AES 加密，并支持[密钥引用](/zh/docs/apisix/terminology/secret/)（`$secret://`）和环境变量（`$env://`）。 |
...
创建一个路由到 LLM 聊天补全端点，使用 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 插件，并配置 `ai-lakera-guard` 插件：
...
创建一个配置了 `ai-lakera-guard` 和 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 插件的路由：
