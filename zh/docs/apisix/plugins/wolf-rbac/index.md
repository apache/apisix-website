...
> 本文介绍了关于 Apache APISIX `wolf-rbac` 插件的基本信息及使用方法。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/wolf-rbac/

## 描述

`wolf-rbac` 插件为 [role-based access control](https://en.wikipedia.org/wiki/Role-based_access_control) 系统提供了添加 [wolf](https://github.com/iGeeky/wolf) 到 Route 或 Service 的功能。此插件需要与 [Consumer](/zh/docs/apisix/terminology/consumer/) 一起使用。
...
| appid         | string | 否     | "unset"                  | 在 `wolf-console` 中已经添加的应用 id。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。                                       |
...
以上接口需要通过 [public-api](/zh/docs/apisix/plugins/public-api/) 插件暴露。
...
你可以使用 [public-api](/zh/docs/apisix/plugins/public-api/) 插件来暴露 API.
