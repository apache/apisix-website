...
> skywalking-logger 将请求和响应日志作为 JSON 对象批量推送到 SkyWalking OAP 服务器，并支持日志格式的自定义。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/skywalking-logger/
...
| log_format             | object  | 否   |          |         | 日志格式以 JSON 的键值对声明。值支持字符串和嵌套对象（最多五层，超出部分将被截断）。字符串中可通过在前面加上 `$` 来引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 或 [NGINX 内置变量](http://nginx.org/en/docs/varindex.html)。 |
| timeout                | integer | 否     | 3                    | [1,...]       | 发送请求后保持连接活动的时间。                                       |
| name                   | string  | 否     | "skywalking logger"  |               | 标识 logger 的唯一标识符。如果你使用 Prometheus 监视 APISIX 指标，名称将以 `apisix_batch_process_entries` 导出。                                         |
...
| max_req_body_bytes | integer | 否 | 524288 | >=1 | 在此大小范围内的请求主体将被记录，如果大小超过配置值则截断后记录。|
...
| max_resp_body_bytes | integer | 否 | 524288 | >=1 | 在此大小范围内的响应主体将被记录，如果大小超过配置值则截断后记录。|

该插件支持使用批处理器来聚合并批量处理条目（日志/数据）。这样可以避免插件频繁地提交数据，默认设置情况下批处理器会每 `5` 秒钟或队列中的数据达到 `1000` 条时提交数据，如需了解批处理器相关参数设置，请参考 [Batch-Processor](/zh/docs/apisix/batch-processor/#配置)。

## 元数据

你还可以通过配置插件元数据来设置日志的格式。可用的配置如下：

| 名称                    | 类型    | 必选项 | 默认值                | 有效值        | 描述                                                               |
| ---------------------- | ------- | ------ | -------------------- | ------------- | ---------------------------------------------------------------- |
| log_format | object | 否    |  | 日志格式以 JSON 的键值对声明。值支持字符串和嵌套对象（最多五层，超出部分将被截断）。字符串中可通过在前面加上 `$` 来引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 或 [NGINX 内置变量](http://nginx.org/en/docs/varindex.html)。 |
| max_pending_entries | integer | 否 | 8192 | >= 1 | 待处理条目数的上限。积压超过该值后新条目会被丢弃，避免日志服务变慢或不可达时 worker 内存无限增长。该上限对应的内存开销参见 [批处理器](/zh/docs/apisix/batch-processor/#限制积压条目数)。 |

## 示例

以下示例演示了如何为不同场景配置 `skywalking-logger` 插件。

要按照示例操作，请按照 [Skywalking 的文档](https://skywalking.apache.org/docs/main/next/en/setup/backend/backend-docker/) 使用 Docker Compose 启动存储、OAP 和 Booster UI。设置完成后，OAP 服务器应在 `12800` 上监听，并且你应该能够通过 [http://localhost:8080](http://localhost:8080) 访问 UI。
...
在 [Skywalking UI](http://localhost:8080) 中，导航至 __General Service__ > __Services__。你应该会看到一个名为 `APISIX` 的服务，其中包含与你的请求相对应的日志条目：
...
首先，使用 `skywalking-logger` 插件创建路由，并使用你的 OAP 服务器 URI 配置插件：
...
你应该收到 `HTTP/1.1 200 OK` 响应。在 [Skywalking UI](http://localhost:8080) 中，导航至 __General Service__ > __Services__。你应该会看到一个名为 `APISIX` 的服务，其中包含与你的请求相对应的日志条目：
...
你应该收到 `HTTP/1.1 200 OK` 响应。在 [Skywalking UI](http://localhost:8080) 中，导航到 __General Service__ > __Services__。你应该看到一个名为 `APISIX` 的服务，其中包含与你的请求相对应的日志条目，并记录了请求正文：
...
在 [Skywalking UI](http://localhost:8080) 中，导航到 __General Service__ > __Services__。你应该会看到一个名为 `APISIX` 的服务，其中包含与你的请求相对应的跟踪，你可以在其中查看相关日志：
