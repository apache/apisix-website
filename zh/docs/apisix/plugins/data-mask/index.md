...
> API 网关 Apache APISIX data-mask 插件可用于在请求数据写入访问日志或日志插件之前，对敏感字段进行掩码或脱敏处理。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/data-mask/
...
若需要在 Nginx 访问日志中记录掩码后的查询参数，需将 `nginx_config.http.access_log_format` 中的 `$request` 替换为 `$request_line`：
