...
> google-cloud-logging 插件将请求和响应日志批量推送到 Google Cloud Logging Service，并支持自定义日志格式。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/google-cloud-logging/
...
| name                    | string        | False  | google-cloud-logging                                                                                                                                                                                   | | 批处理器中插件的唯一标识符。如果使用 [Prometheus](/zh/docs/apisix/plugins/prometheus/) 监控 APISIX 指标，该名称将在 `apisix_batch_process_entries` 中导出。                                                                                                                                                              |
...
注意：schema 中还定义了 `encrypt_fields = {"auth_config.private_key"}`，这意味着该字段将会被加密存储在 etcd 中。具体参考[加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。
...
| 名称               | 类型    | 必选项 | 默认值 | 有效值 | 描述                                                                                                                                                                                                 |
|--------------------|---------|--------|--------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| log_format         | object  | False  |        |        | 以 JSON 键值对形式声明的自定义日志格式。值可通过 `$` 前缀引用 [APISIX 变量](/zh/docs/apisix/apisix-variable/) 或 [NGINX 变量](https://nginx.org/en/docs/http/ngx_http_core_module.html)。该配置全局生效，对所有绑定 `google-cloud-logging` 的路由和服务生效。 |
