...
> 本篇文档介绍了 Apache APISIX auth-casdoor 插件的相关信息。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/authz-casdoor/
...
注意：schema 中还定义了 `encrypt_fields = {"client_secret"}`，这意味着该字段将会被加密存储在 etcd 中。具体参考 [加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。
