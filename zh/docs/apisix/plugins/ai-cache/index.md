...
> ai-cache 插件将 LLM 响应缓存在 Redis 中，并在后续解析到相同提示词的请求中重放这些响应，从而降低上游的 Token 消耗与延迟。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/ai-cache/
...
`ai-cache` 插件必须与 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 或 [`ai-proxy-multi`](/zh/docs/apisix/plugins/ai-proxy-multi/) 插件一起使用。
...
使用 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 和 `ai-cache` 插件创建一个指向 LLM 聊天补全端点的路由。
