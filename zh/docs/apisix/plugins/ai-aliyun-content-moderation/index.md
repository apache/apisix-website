...
> ai-aliyun-content-moderation 插件集成了阿里云机器辅助审核 Plus，用于在代理 LLM 请求时检查请求和响应内容的风险等级，拒绝超过配置阈值的请求。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/ai-aliyun-content-moderation/
...
`ai-aliyun-content-moderation` 插件应与 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 或 [`ai-proxy-multi`](/zh/docs/apisix/plugins/ai-proxy-multi/) 插件配合使用以代理 LLM 请求。
...
创建一个路由到 LLM 聊天补全端点，使用 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 插件，并在 `ai-aliyun-content-moderation` 插件中配置集成详情以及 `deny_code` 和 `deny_message`：
...
创建一个配置了 `ai-aliyun-content-moderation` 和 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 插件的路由：
...
创建一个路由到 LLM 聊天补全端点，使用 [`ai-proxy`](/zh/docs/apisix/plugins/ai-proxy/) 插件，并将 `ai-aliyun-content-moderation` 中的 `risk_level_bar` 配置为 `high`：
