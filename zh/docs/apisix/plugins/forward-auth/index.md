...
> forward-auth 插件集成外部授权服务，增强 API 安全性和访问控制。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/forward-auth/
...
要跟随前两个示例操作，请提前搭建好外部授权服务，或使用 [serverless-pre-function](/zh/docs/apisix/plugins/serverless/) 插件创建如下所示的模拟认证服务：
...
请提前搭建好外部授权服务，或使用 [serverless-pre-function](/zh/docs/apisix/plugins/serverless/) 插件创建模拟认证服务。该函数检查 `tenant_id` 请求头是否为 `123`，如果是则返回 `200 OK`，否则返回 403 和错误消息。
