...
> serverless-pre-function 和 serverless-post-function 插件支持在 APISIX 的指定阶段动态运行 Lua 函数。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/serverless/

## 描述

无服务器函数由两个插件组成：`serverless-pre-function` 和 `serverless-post-function`。这些插件支持在[执行阶段](/zh/docs/apisix/terminology/plugin/#plugins-execution-lifecycle)的开始和结束时执行用户定义的逻辑。
...
以下示例演示如何配置 serverless 插件，在 `rewrite` [阶段](/zh/docs/apisix/terminology/plugin/#plugins-execution-lifecycle)之前和之后执行自定义逻辑，将信息记录到错误日志中。
