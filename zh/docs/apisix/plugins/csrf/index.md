...
> CSRF 插件基于 Double Submit Cookie 的方式，帮助用户阻止跨站请求伪造攻击。

Source: https://apisix.apache.org/zh/docs/apisix/plugins/csrf/
...
注意：schema 中还定义了 `encrypt_fields = {"key"}`，这意味着该字段将会被加密存储在 etcd 中。具体参考 [加密存储字段](/zh/docs/apisix/plugin-develop/#加密存储字段)。
