...
Source: https://apisix.apache.org/zh/docs/apisix/mtls/
...
你需要构建 [APISIX-runtime](/zh/docs/apisix/FAQ/#如何构建-APISIX-runtime-环境？)，并且需要在配置文件中设定 `etcd.tls` 来使 ETCD 的双向认证功能正常工作。
...
我们提供了一个[演示教程](/zh/docs/apisix/tutorials/client-to-apisix-mtls/)，详细地讲解了如何配置客户端和 APISIX 之间的 mTLS。

在配置 `ssl` 资源时，同时需要配置 `client.ca` 和 `client.depth` 参数，分别代表为客户端证书签名的 CA 列表，和证书链的最大深度。可参考：[SSL API 文档](/zh/docs/apisix/admin-api/#ssl)。
...
在配置 upstream 资源时，可以使用参数 `tls.client_cert` 和 `tls.client_key` 来配置 APISIX 用于与上游进行通讯时使用的证书。可参考 [Upstream API 文档](/zh/docs/apisix/admin-api/#upstream)。

该功能需要 APISIX 运行在 [APISIX-Runtime](/zh/docs/apisix/FAQ/#如何构建-apisix-runtime-环境) 上。
