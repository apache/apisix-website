...
Source: https://apisix.apache.org/zh/docs/apisix/plugin-develop/

此文档是关于 lua 语言的插件开发，其他语言请看：[external plugin](/zh/docs/apisix/external-plugin/)。
...
通过函数输入参数 **schema_type** 可以对不同类型的 schema 进行对应的校验。例如很多插件需要使用一些[元数据](/zh/docs/apisix/terminology/plugin-metadata/)，可以定义插件的 `metadata_schema`。
...
再比如 [key-auth](https://github.com/apache/apisix/blob/master/apisix/plugins/key-auth.lua) 插件为了跟 [Consumer](/zh/docs/apisix/admin-api/#consumer) 资源一起使用，认证插件需要提供一个 `consumer_schema` 来检验 `Consumer` 资源的 `plugins` 属性里面的配置。
...
根据业务功能，确定你的插件需要在哪个[阶段](/zh/docs/apisix/terminology/plugin/#插件执行生命周期)执行。
...
如果你的插件，涉及到一些外部的依赖和三方库，请首先检查一下依赖项的内容。如果插件需要用到共享内存，需要在 [自定义 Nginx 配置](/zh/docs/apisix/customize-nginx-configuration/)中声明，例如：
...
注意，注册的接口将不会默认暴露，需要使用[public-api 插件](/zh/docs/apisix/plugins/public-api/)来暴露它。

## 注册控制接口

如果你只想暴露 API 到 localhost 或内网，你可以通过 [Control API](/zh/docs/apisix/control-api/) 来暴露它。
...
[查看更多有关 control API 介绍](/zh/docs/apisix/control-api/)
