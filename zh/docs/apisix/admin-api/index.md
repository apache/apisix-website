...
> 本文介绍了 Apache APISIX Admin API 支持的功能，你可以通过 Admin API 来获取、创建、更新以及删除资源。

Source: https://apisix.apache.org/zh/docs/apisix/admin-api/

## 描述 {#description}

Admin API 是一组用于配置 Apache APISIX 路由、上游、服务、SSL 证书等功能的 RESTful API。

你可以通过 Admin API 来获取、创建、更新以及删除资源。同时得益于 APISIX 的热加载能力，资源配置完成后 APISIX 将会自动更新配置，无需重启服务。如果你想要了解其工作原理，请参考 [Architecture Design](/zh/docs/apisix/architecture-design/apisix/)。
...
| 名称             | 必选项                            | 类型     | 描述                                                                                                                                                                                                                                                                                    | 示例值                                                 |
| ---------------- | -------------------------------- | -------- |---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| ---------------------------------------------------- |
| uri              | 是，与 `uris` 二选一。       | 匹配规则 | 除了如 `/foo/bar`、`/foo/gloo` 这种全量匹配外，使用不同 [Router](/zh/docs/apisix/terminology/router/) 还允许更高级匹配，更多信息请参考 [Router](/zh/docs/apisix/terminology/router/)。                                                                                                                                                 | "/hello"                                             |
| uris             | 是，不能与 `uri` 二选一。        | 匹配规则 | 非空数组形式，可以匹配多个 `uri`。                                                                                                                                                                                                                                                               | ["/hello", "/world"]                                 |
| plugins          | 否                               | Plugin   | Plugin 配置，请参考 [Plugin](/zh/docs/apisix/terminology/plugin/)。                                                                                                                                                                                                                                            |                                                      |
| script           | 否                               | Script   | Script 配置，请参考 [Script](/zh/docs/apisix/terminology/script/)。                                                                                                                                                                                                                                            |                                                      |
| upstream         | 否                               | Upstream | Upstream 配置，请参考 [Upstream](/zh/docs/apisix/terminology/upstream/)。                                                                                                                                                                                                                              |                                                      |
| upstream_id      | 否                               | Upstream | 需要使用的 Upstream id，请参考 [Upstream](/zh/docs/apisix/terminology/upstream/)。                                                                                                                                                                                                                     |                                                      |
| service_id       | 否                               | Service  | 需要绑定的 Service id，请参考 [Service](/zh/docs/apisix/terminology/service/)。                                                                                                                                                                                                                        |                                                      |
| plugin_config_id | 否，不能与 Script 共同使用。      | Plugin   | 需要绑定的 Plugin Config id，请参考 [Plugin Config](/zh/docs/apisix/terminology/plugin-config/)。                                                                                                                                                                                                           |                                                      |
...
| ---------------- | ----------------------- | -------- | ---------------------------------------------------------------- | ------------------------------------------------ |
| plugins          | 否                      | Plugin   | Plugin 配置，请参考 [Plugin](/zh/docs/apisix/terminology/plugin/)。               |                                                  |
| upstream         | 与 `upstream_id` 二选一。  | Upstream | Upstream 配置，请参考 [Upstream](/zh/docs/apisix/terminology/upstream/)。         |                                                  |
| upstream_id      | 与 `upstream` 二选一。    | Upstream | 需要使用的 upstream id，请参考 [Upstream](/zh/docs/apisix/terminology/upstream/)。|                                                  |
| name             | 否                     | 辅助     | 服务名称。                                                      |                                             |
| desc             | 否                     | 辅助     | 服务描述。                                                          |                                                  |
| labels           | 否                     | 匹配规则 | 标识附加属性的键值对。                                                 | {"version":"v2","build":"16","env":"production"} |
...
| service_name   | 是，与 `nodes` 二选一。                              | string         | 服务发现时使用的服务名，请参考 [集成服务发现注册中心](/zh/docs/apisix/discovery/)。                                                                                                                                                                                                                                                                                            | `a-bootiful-client`                              |
| discovery_type | 是，与 `service_name` 配合使用。                      | string         | 服务发现类型，请参考 [集成服务发现注册中心](/zh/docs/apisix/discovery/)。                                                                                                                                                                                                                                                                                                      | `eureka`                                         |
...
| checks         | 否                                             | health_checker | 配置健康检查的参数，详细信息请参考 [health-check](/zh/docs/apisix/tutorials/health-check/)。                                                                                                                                                                                                                                                                                               |                                                  |
...
以下特性需要 APISIX 运行于 [APISIX-Runtime](/zh/docs/apisix/FAQ/#如何构建-APISIX-Runtime-环境？)：
...
| 名称        | 必选项 | 类型           | 描述                                                                                                   | 示例                                             |
| ----------- | ------ | -------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------ |
| cert        | 是     | 证书           | HTTP 证书。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。                                                                                             |                                                  |
| key         | 是     | 私钥           | HTTPS 证书私钥。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。                                                                                         |                                                  |
| certs       | 否   | 证书字符串数组 | 当你想给同一个域名配置多个证书时，除了第一个证书需要通过 `cert` 传递外，剩下的证书可以通过该参数传递上来。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。 |                                                  |
| keys        | 否   | 私钥字符串数组 | `certs` 对应的证书私钥，需要与 `certs` 一一对应。该字段支持使用 [APISIX Secret](/zh/docs/apisix/terminology/secret/) 资源，将值保存在 Secret Manager 中。                                                          |                                                  |
...
| ssl_protocols | 否    | tls 协议字符串数组               | 用于控制服务器与客户端之间使用的 SSL/TLS 协议版本。更多的配置示例，请参考[SSL 协议](/zh/docs/apisix/ssl-protocol/)。                                  |                `["TLSv1.1", "TLSv1.2", "TLSv1.3"]`                                  |
...
更多的配置示例，请参考[证书](/zh/docs/apisix/certificate/)。
...
| 名称        | 必选项 | 类型   | 描述                                               | 示例值       |
| ----------- | ------ | ------ | ------------------------------------------------- | ---------- |
| plugins     | 是     | Plugin | 插件配置。详细信息请参考 [Plugin](/zh/docs/apisix/terminology/plugin/)。 |            |
...
| 名称      | 必选项  | 类型 | 描述        | 示例值 |
|---------  |---------|----|-----------|----|
|plugins    | 是      |Plugin| 更多信息请参考 [Plugin](/zh/docs/apisix/terminology/plugin/)。||
...
Stream Route 是用于 TCP/UDP 动态代理的路由。详细信息请参考 [TCP/UDP 动态代理](/zh/docs/apisix/stream-proxy/)。
...
| upstream         | 否    | Upstream | Upstream 配置，详细信息请参考 [Upstream](/zh/docs/apisix/terminology/upstream/)。             |         |
| upstream_id      | 否    | Upstream | 需要使用的 Upstream id，详细信息请 [Upstream](/zh/docs/apisix/terminology/upstream/)。       |         |
| service_id       | 否    | String   | 需要使用的 [Service](/zh/docs/apisix/terminology/service/) id.                   |                               |
...
你可以查看 [Stream Proxy](/zh/docs/apisix/stream-proxy/#更多-route-匹配选项) 了解更多过滤器的信息。
