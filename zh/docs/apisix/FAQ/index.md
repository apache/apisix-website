...
> 本文列举了使用 Apache APISIX 时常见问题解决方法。

Source: https://apisix.apache.org/zh/docs/apisix/FAQ/
...
- 完全动态
- 支持[热加载插件](/zh/docs/apisix/terminology/plugin/#热加载)。

## APISIX 所展现出的性能如何？

与其它 API 网关相比，Apache APISIX 提供了更好的性能，其单核 QPS 高达 18,000，平均延迟仅为 0.2 ms。

如果您想获取性能基准测试的具体结果，请查看 [benchmark](/zh/docs/apisix/benchmark/)。
...
如果你想开发符合自身业务逻辑的插件，请参考：[如何开发插件](/zh/docs/apisix/plugin-develop/)。
...
1. 创建一个[Route](/zh/docs/apisix/terminology/route/)并配置 `vars` 字段：
...
2、通过 [traffic-split](/zh/docs/apisix/plugins/traffic-split/) 插件来实现。
...
1. 在 [redirect](/zh/docs/apisix/plugins/redirect/) 插件中将 `http_to_https` 设置为 `true`：
...
如果你想了解更多关于热加载的内容，请参考[热加载](/zh/docs/apisix/terminology/plugin/#热加载)。
...
这是由于缺少 OpenResty openssl 开发工具包。要安装它，请参考[installation dependencies](/zh/docs/apisix/install-dependencies/)。
