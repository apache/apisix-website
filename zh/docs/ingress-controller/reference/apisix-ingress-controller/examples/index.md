...
Source: https://apisix.apache.org/zh/docs/ingress-controller/reference/apisix-ingress-controller/examples/
...
❷ The `port` in the Gateway listener is used for routing matching based on [`listener_port_match_mode`](/docs/ingress-controller/reference/configuration-file/) (`off` by default; `auto` or `explicit` opt in). The controller cannot dynamically open new ports on the data plane, so ensure APISIX is configured to listen on the port.
