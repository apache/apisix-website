# 2025 社区月报 (03.01 - 03.31)

> Apache APISIX 社区的月报旨在帮助社区成员更全面地了解社区的最新动态，方便大家参与到 Apache APISIX 社区中来。

Source: https://apisix.apache.org/zh/blog/2025/03/31/march-monthly-report/
...
### 1. 新增 `ai-prompt-guard` 插件

相关 PR：https://github.com/apache/apisix/pull/12008
...
### 2. 新增 `ai-rate-limiting` 插件

相关 PR：https://github.com/apache/apisix/pull/12037
...
### 3. 新增 `ai-request-rewrite` 插件

相关 PR：https://github.com/apache/apisix/pull/12036

贡献者：[LiteSun](https://github.com/LiteSun)

`ai-request-rewrite` 插件利用预定义的提示和 AI 服务，能够智能地修改客户端请求，从而实现在转发至上游服务之前借助 AI 完成内容转换。

### 4. 支持代理 OpenAI 兼容的大型语言模型

相关 PR：https://github.com/apache/apisix/pull/12004

贡献者：[shreemaan-abhishek](https://github.com/shreemaan-abhishek)

此 PR 引入了一个新的提供者 `openai-compatible`，能够代理请求到与 OpenAI 兼容的大型语言模型。

### 5. 在 `jwt-auth` 插件中将 JWT 存储到请求上下文中

相关 PR：https://github.com/apache/apisix/pull/11675

贡献者：[mikyll](https://github.com/mikyll)
...
相关 PR：https://github.com/apache/apisix/pull/11987
...
### 7. 将 `ssl_trusted_certificate` 中的默认值设置为 `system`

相关 PR：https://github.com/apache/apisix/pull/11993
...
### 8. 在 `openid-connect` 插件中增加 `valid_issuers` 字段
...
### 9. 在 `ai-proxy-multi` 插件中引入与 `ai-rate-limiting` 插件相匹配的回退策略
...
### 10. 在 `ai-proxy` 插件中支持代理嵌入式 API

相关 PR：https://github.com/apache/apisix/pull/12062

贡献者：[shreemaan-abhishek](https://github.com/shreemaan-abhishek)

此 PR 为 `ai-proxy` 插件增加了对代理嵌入式 API 的支持。

### 11. 在 ip-restriction 插件中支持 404 响应码

相关 PR：https://github.com/apache/apisix/pull/12076
...
相关 PR：https://github.com/apache/apisix/pull/12029

贡献者：[papdaniel](https://github.com/AlyHKafoury)
